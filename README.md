...
- [**Supervisor Agent**](backend/agents/supervisor/README.md): Performs a lightweight complexity check on user inputs to handle pathing and route requests through the appropriate pipeline.
- [**Parameter Agent**](backend/agents/parameter/README.md): Extracts necessary geometric parameters and identifies missing constraints using cloud LLMs.
- [**Planner Agent**](backend/agents/planner/README.md): Breaks down complex shapes into logical construction steps.
- [**Code Generator Agent**](backend/agents/code-generator/README.md): Translates the plan and parameters into executable FreeCAD Python scripts using cloud AI models.
- [**Code Executor (Hardcoded)**](backend/agents/execution/README.md): A deterministic, hardcoded code execution service that directly interfaces with FreeCAD to execute scripts and evaluate runtime output.
...
- **Complexity Check ([Supervisor Agent](backend/agents/supervisor/README.md))**: Performs a fast, simple complexity analysis to determine pathing—deciding whether a prompt requires full multi-step planning or a streamlined generation flow.
- **Parameter Gathering**: The [Parameter Agent](backend/agents/parameter/README.md) reasons about the user's request to identify explicit and implicit dimensions. If parameters are missing, it tracks them, preventing premature code generation.

### 4. Planning
For intricate designs, the [**Planner Agent**](backend/agents/planner/README.md) steps in to decompose the instruction into logical step categories (e.g., base sketch, extrusion, boolean operations). This planning phase provides a structured blueprint that constrains the code generation, significantly improving reliability over single-prompt generation.

### 5. Action-Taking and Self-Healing
Once the code is generated, the hardcoded [**Code Executor**](backend/agents/execution/README.md) takes action by running the Python script directly inside FreeCAD. 
